        ...
        for det in pred:
            if det is not None and len(det):  # 检查检测结果是否为空
                ...
                with open(txt_path, 'r') as f:
                    truth_label = f.readlines()
                    truth_cls = int(truth_label[0].split()[0])

                label_parts = label.split()  # 分割字符串，默认使用空格进行分割
                pred_cls = int(label_parts[0][1:-1])  # 提取数字部分，并转换为整数类型

                if truth_cls != pred_cls:
                    # Save image
                    # 将带有边界框的图像保存到指定的输出文件夹中
                    save_path = str(Path(output) / Path(path).name)  # 创建保存路径
                    print(truth_cls, pred_cls, save_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='wrong', help='path to output directory')
